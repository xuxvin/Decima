                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDIA ought to be enough for anybody." - Bill Gates, 2017
    ui->zDIApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDIASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDIA->setEnabled(false);
        ui->pushButtonMintzDIA->setToolTip(tr("zDIA is currently disabled due to maintenance."));

        ui->pushButtonSpendzDIA->setEnabled(false);
        ui->pushButtonSpendzDIA->setToolTip(tr("zDIA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDIApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDIA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDIA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDIA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDIA in ") +
...
void PrivacyDialog::on_pushButtonSpendzDIA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDIA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDIA
        sendzDIA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDIA
    sendzDIA();
}

void PrivacyDialog::on_pushButtonZDiaControl_clicked()
{
    ZDiaControlDialog* zDiaControl = new ZDiaControlDialog(this);
    zDiaControl->setModel(walletModel);
    zDiaControl->exec();
}

void PrivacyDialog::setZDiaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDIASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDIA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Decima Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDIApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDIA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DIA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDIA</b>";
    ...
    // use mints from zDia selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDiaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDiaControlDialog::GetSelectedMints();
    }
    ...
    // Clear zdia selector in case it was used
    ZDiaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDia Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Dia, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDia Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDIA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDIA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDIA "));
    ui->labelzDIAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDIASyncStatus->setVisible(fShow);
}
