...
#include "ui_zdiacontroldialog.h"
...
std::list<std::string> ZDiaControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDiaControlDialog::listMints;

ZDiaControlDialog::ZDiaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDiaControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDiaControlDialog::~ZDiaControlDialog()
{
    delete ui;
}

void ZDiaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDiaControlDialog::updateList()
{
...
void ZDiaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDiaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDia_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDiaControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDiaControlDialog::GetSelectedMints()
{
...
void ZDiaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
